...
FOREIGN KEY (answer) REFERENCES Post(pid)
);
CREATE TABLE Post(
...
so the user can´t input empty strings.','int size = 0;
do {
   puts("Insert the ID?");
...
Insert into Topic (title,createtime,creator,likes,tags,description,code) values ('How do I properly compare strings?',datetime('now'),1,68,'String compare','I am trying to get a program to let a user enter a word or character, 
...
Am I comparing the two incorrectly? My code looks like this:','#include <stdio.h>
...
*/
Insert into Article (title,likes,createtime,creator,tag,content) 
values ('Strlen is return 1 on empty strings - fgets',66,datetime('now'),3,'fgets',
